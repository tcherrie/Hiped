...
#plt.figure()
f1v.plot(label3D = False) # matplotlib 3D rendering not so good for multiple surface plot...
plt.show()
...
interpVector= hp.Interpolation(domain = domain, children = [f1v,f2v,f3v,f4v,f5v],
                               label = "interpVector", penalization = penal)

#%% 4) Coordinates x in the polygon and scalar field u

n = 100
x = interpVector.setInitialVariable(n, typeInit = "rand", radius = 1) # initialization of the variables
...
for i in range(10):
    ...
    xPert[l] = x[l] + pert
    xPert = interpVector.projection(xPert)
    pert = xPert[l] - x[l]
